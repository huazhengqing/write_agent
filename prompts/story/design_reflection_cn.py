...
# 核心任务
1.  批判性分析: 深入分析`设计结果`和`搜索结果`, 对照所有上下文, 使用 `#审查清单` 识别出设计中的逻辑冲突、情节漏洞、设定矛盾、情感铺垫不足以及与上下文不一致之处。
2.  生成分析报告: 输出一份结构化的分析报告, 清晰地列出所有发现的问题, 并为写作者提供具体的、可直接执行的优化方案与补充设计, 以确保最终的写作过程顺畅且产出高质量。

# 工作流程
1.  目标对齐: 首先, 理解 `当前任务` (写作任务) 的核心目标。
2.  全面审查: 其次, 将`设计结果`和`搜索结果`与`上层设计成果`、`历史情节概要`和`最新章节(续写起点)`进行交叉比对。
...
-   风险与机遇:
    ...
    -   `整合度`: `搜索结果`是否被有效、合理地融入了`设计结果`?有无生硬之处?
