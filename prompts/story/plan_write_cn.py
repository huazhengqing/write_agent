...
- 创建最终 `write` 任务, 依赖所有同层级的 `design` 和 `search` 任务, 继承父任务 `goal` 和 `length`, 本次不分解。

## 当`设计方案`包含结构规划成果
- 解析 `结构规划` 定义的下一层级单元 (如卷、幕、章)。
- 为每个单元创建 `write` 子任务, 精确映射 `hierarchical_position` 、`goal` 和 `length`。
- `goal` 必须详细具体, 明确包含该单元在`设计方案`中的核心使命、情节概要和关键要素, 禁止创造`设计方案`不存在的情节。
- 必需产出多个串行 `write` 子任务。
