...
## 当`设计方案`不含结构规划成果
- 产出: `design`/`search` 子任务组 + 一个占位 `write` 子任务。
- 依据 `#设计任务清单` 的规则, 生成当前层级标准的 `design` 任务。
- 为触发点创建额外 `design`/`search` 任务, 并将其整合进子任务序列中, 并设定其依赖。
- 创建最终 `write` 子任务, 依赖所有同层级的 `design` 和 `search` 任务, 继承父任务的 `hierarchical_position` 、`goal` 和 `length`, 本次不分解。

## 当`设计方案`包含结构规划方案
- 解析结构规划方案中定义的下一层级单元 (如卷、幕、章)。
- 为每个单元创建 `write` 子任务, 精确映射 `hierarchical_position` 、`goal` 和 `length`。
- `goal` 必须详细具体, 明确包含该单元在结构规划方案中的核心使命、情节概要和关键要素, 禁止创造`设计方案`不存在的情节。
- 必需产出多个 `write` 子任务, 每个子任务的 `dependency`、`sub_tasks` 必须为空。

# 设计任务清单
- 清单是规划的最小集, 需基于上下文扩展。
- 逐项处理: 必须为清单每项创建独立 `design` 任务。任务的 `goal` 必须完整复制清单中的描述, 禁止合并、遗漏、删减、概括。
- 依赖设定: 根据任务逻辑，为 `dependency` 设定同层级的`design`/`search`前置任务。
- 补充要素: 结合上下文, 为任务补充设计要素与约束。
{task_level}
...
- 字段:
    ...
    - `dependency`: 同层级的前置`design`/`search`任务ID列表。
...
{{
    ...
    "sub_tasks": [
        ...
        {{
            ...
            "task_type": "write",
            "hierarchical_position": "全书",
            "goal": "[占位写作任务]: 根据[所有同层级设计任务], 继承父任务目标",
            "dependency": ["1.1", "1.2"],
            "length": "1000000字",
            "sub_tasks": []
        }}
