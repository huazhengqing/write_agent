...
- 当前任务是根任务, 即全本小说写作任务。
- 规划流程的第3步, 对第2步(critic.py)生成的内容进行精练。
"""
...
# 任务
综合`规划草案`和`质询建议`, 更新`当前任务`的JSON表示, 将新规划的子任务作为`sub_tasks`嵌入其中。
...
## 生成JSON
- 构建父任务: 将`当前任务`作为JSON的根对象。在`reasoning`字段中, 简要说明你是如何综合双方意见并做出最终决策的。
- 创建子任务: 基于整合优化后的最终方案, 为每一个新规划的设计任务创建一个JSON对象。
- 精确映射: 将最终方案中的要素精确映射到每个子任务的JSON字段中, 并为每个任务预估`complexity_score`。
- 嵌入并输出: 将所有子任务JSON对象组成的列表, 赋值给父任务的`sub_tasks`字段。你的唯一输出必须是这个结构严谨的、包含父子关系的纯JSON对象, 禁止任何创造性发挥或内容修改。
