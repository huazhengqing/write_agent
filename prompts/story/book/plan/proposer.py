...
- 职责: 规划"做什么"(What), 而非设计"如何做"(How)。即决定"为了达到顶级水准, 我们现在最应该做什么事?"。
- 产出一系列定义清晰、目标明确的design任务, 而不是设计本身。
- "战略思考框架"的问题: 审视现状、发现短板、并最终定义出"需要做什么任务"的思维工具。
- 迭代能力: 本任务支持多次调用。每次调用可以规划一个或多个子任务, 直到父任务可被直接完成或可被结构划分。
- Proposer Agent 不被强制一次性规划所有子任务, 而是可以根据需要, 一次只生成一个或少数几个高质量、高细节的子任务, 并且支持多次调用以完成整个任务的规划。
# 要求
- 禁止提供清单, 这会限制AI的思考
