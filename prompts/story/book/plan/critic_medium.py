...
## 生成评审报告
- 将所有任务的“强化方案”汇总，按照`#输出`格式要求，生成最终的评审报告。

# 输出
- 格式: Markdown。
- 结构: 针对`规划草案`中的每个任务，按以下格式输出评审意见。
    #### 针对“任务标题”的评审
    - 核心问题: [综合所有质询，指出任务定义中最根本的缺陷或与战略目标的偏差。]
    - 强化建议: [提出具体的、可操作的修改方案。例如：将`核心目标`修改为...；在`具体指令`中增加...；将`验收标准`强化为...]
    (如果对同个任务有多个质询点，可重复使用此格式)
"""
