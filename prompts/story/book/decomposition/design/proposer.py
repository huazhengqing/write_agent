...
- 定位: 明确本次分解如何服务于故事的产品定位(题材、卖点、目标读者)。

## 构思分解维度
- 发散思考: 基于任务目标和产品定位, 创造性地提出完成父任务所需的核心设计维度。
- 创建Search任务: 仅当构思依赖外部客观事实时, 才创建`search`任务。


## 注入独特性与记忆点
- 识别陈词滥调: 审视所有维度, 识别潜在的常见套路或同质化设计。
- 强化记忆点: 为每个维度注入独特的"钩子"或"反直觉设定", 并融入设计要求。

## 审查与筛选
- 正交性: 确保维度间逻辑独立、互不重叠。合并相似维度, 拆分复合维度。
- 完整性: 确保所有维度能完整覆盖父任务的核心目标, 无遗漏。
- 粒度: 确保各维度工作量均衡, 且只分解一级。

## 构建提案
- 转化: 将最终确定的每个维度, 转化为一个具体的子任务。
- 填充: 严格按照`# 任务结构`, 为每个子任务撰写所有必需字段。
