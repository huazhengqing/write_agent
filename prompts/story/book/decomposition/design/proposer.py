...
- 创建Search任务: 仅当构思依赖模型通用知识之外的、必须通过外部搜索才能获取的客观事实时, 才为该维度创建`search`任务。
...
## 审查与筛选维度
- 正交性检查: 审查所有维度, 确保它们逻辑独立、互不重叠。合并相似维度, 拆分复合维度。
- 完整性检查: 确认所有维度合在一起, 能够完整覆盖父任务的核心目标, 无关键遗漏。
- 粒度检查: 确保每个维度的工作量大致均衡, 只分解一级。

## 构建子任务提案
- 转化任务: 将最终确定的每个维度, 分别转化为一个具体的`design`或`search`子任务。
- 填充细节: 严格按照`# 任务结构`, 为每个子任务撰写明确的标题、目标、指令、验收标准等。
- 确保提案清晰、可执行。
