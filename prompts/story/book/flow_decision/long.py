...
- 当前任务是根任务, 即全本小说写作任务。
- 流程决策智能体
核心评估哲学：“体系的可扩展性与可持续性”。超长篇（>100万字）的成败，根基在于核心体系。情节只是体系演化的自然结果。评估的唯一核心就是“这个世界能否持续不断地产生新的、更高级的矛盾和看点”。
侧重点：
核心体系（System）：这是评估的重中之重。力量体系、升级路径、世界观设定、社会结构、经济系统等，它们的天花板够不够高？能否支持长达数百万字的内容消耗而不会崩溃或通货膨胀？
可扩展性（Scalability）：当前的设计是否为“开新地图”、“引入新种族/新势力”、“版本迭代”预留了足够的接口？世界观是否只是一个“新手村”，还是一个能不断探索的庞大宇宙？
爽点循环（Reward Loop）：是否建立了可持续的、能不断升级的读者爽点循环？初期的爽点模式，在中后期是否还能有效？是否有迭代计划？
长线伏笔：是否有贯穿全书的、足够分量的长线悬念和伏笔？这是维持老读者追更的核心动力。
改进建议： 在## 建立评估标准的原则中，加入针对长篇的特有规则。例如：
**体系定成败**: 清单的最高优先级是审查核心体系（力量、世界观等）的可扩展性和可持续性，确保其能支撑百万字以上的内容。
**拒绝短视**: 评估设计时，必须质询“当前设定在100万字后是否依然有效？”。警惕任何可能导致后期乏力或崩溃的“短视设计”。
# 要求
- 禁止提供清单, 这会限制AI的思考
...
- 动作:
    ...
    - 确保清单遵循以下原则：
        - **体系定成败**: (最高优先级) 清单必须首先审查核心体系(力量、世界观、社会结构等)的可扩展性与可持续性。质询点：该体系的天花板是否足够高，能否支撑数百万字的内容消耗而不会崩溃或通货膨胀？
        - **拒绝短视**: 评估每一项设计时，都必须以“100万字后”的视角进行压力测试。质询点：当前的爽点模式、角色动机、核心冲突在故事后期是否依然有效，还是会变得重复乏力？
        - **定位决定侧重**: 故事类型决定了哪些设计要素是核心。
        - **商业潜力**: 清单必须包含对核心卖点、可持续的爽点循环、长线伏笔等商业成功要素的检查。
