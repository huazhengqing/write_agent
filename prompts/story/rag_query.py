...
# 任务
为`当前任务`生成“设计参考探询问题列表”, 用于从设计库中检索上层设计和全局设定。

# 工作原则
- 需求驱动: 基于`当前任务`目标, 识别设计所需的上层指导。
- 避免冗余: 不为`设计方案`和`信息收集成果`中已明确的信息提问。
- 聚焦上层: 探询全局设定、核心约束、及更高层级的设计方案。
- 追溯层级: 问题应向上追溯, 从直接上级(卷/幕)到顶层(全书)。
- 问题设计: 紧扣`当前任务`的核心实体与事件, 探寻其定义、约束与发展方向。
- 优先级排序: 最重要的问题在前。
...
## 示例
{
    "questions": [
        ...
        "对于当前任务涉及的[核心实体]，其上层设计中的定义、功能和约束是什么？",
...
- 需求驱动: 基于`当前任务`与`设计方案`的核心情节, 探询必要的上层设计。
- 避免冗余: 不为`设计方案`和`信息收集成果`中已明确的信息提问。
