...
# 核心任务
针对`当前设计任务`, 运用`#知识库`中的原则, 构建一套独特的、可执行的`设计指南`。
你的任务是**规划如何做 (Plan How)**, 而不是**直接做 (Do)**。
...
# 工作流程
1.  **分析与原则选择**:
    - 深入分析`当前设计任务`的目标与`上层设计方案`中的`[核心锚点]`。
    - 从`#知识库`中, 选择3-5条最相关的原则作为理论基础。
    - **强制要求**: 必须包含至少2条`#创新策略`中的原则。
2.  **构建第一性原理公式**:
    - 基于所选原则, 创造一个全新的、一次性的抽象逻辑公式。
    ...
    - 公式构建元素 (仅用于启发, 禁止照抄):
        - 驱动力: `[初始缺陷]`, `[核心动机]`, `[外部压力]`, `[内在欲望]`, `[复仇渴望]`, `[守护誓言]`, `[探寻真相]`
        ...
        - 跨界模型: `[供需关系]`, `[沉没成本]`, `[演化压力]`, `[信息不对称]`
3.  **生成指南**:
    - **核心设计目标**: 用一句话精准概括本次设计的最终目的和必须达成的效果。
    - **第一性原理公式**: 明确列出上一步构建的抽象逻辑公式。
    - **关键要素清单**: 基于公式, 拆解出为完成设计必须定义的具体元素。
    - **创作原则与检查表**: 将公式的推论, 转化为3-5条可操作、可检查的创作指令。
    - **风险预警**: 指出此设计可能存在的风险或需要规避的常见套路。
...
- 纯粹性:  只输出“设计指南”, 无元注释、解释、代码块标记, 禁止包含任何具体的设计内容或情节。
- 结构: 必须包含以下标题
    - `### 核心设计目标`
    - `### 第一性原理公式`
    - `### 关键要素清单`
