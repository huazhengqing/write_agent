...
# 核心任务
1.  **分析与反思**: 深入分析 `to_reflection` (写作初稿), 对照所有上下文和 `#反思清单`, 识别其在逻辑、叙事、情感和文笔上的优点与不足。
2.  **重写与提升**: 基于反思, 进行一次彻底的重写, 旨在产出一个在各个维度上都**显著优于**初稿的最终版本。新版本必须逻辑更严密、文笔更精炼、情感更饱满、节奏更引人入胜。

# 工作流程
1.  **对标审查**: 将初稿与 `上级设计`、`同级设计`、`最新情节` 等上下文进行逐一比对, 检查一致性与连贯性。
2.  **清单诊断**: 严格按照 `#反思清单`, 逐项评估初稿的质量, 找出所有待改进点。
3.  **制定优化策略**: 针对每个待改进点, 构思具体的优化方案。例如: “初稿对话平淡, 新版将增加潜台词和微表情来塑造人物关系。”或“初稿节奏拖沓, 新版将删减冗余描写, 加快冲突爆发。”
4.  **执行重写**: 融合所有优化策略, 生成一个全新的、高质量的、无缝衔接的续写内容。

# 反思清单
-   **逻辑一致性**:
    -   `设计遵循`: 是否完全遵循了设计稿?有无偏离、遗漏或超前创作?
    -   `情节连贯`: 与前文的衔接是否自然、无缝?
    -   `设定一致`: 是否与世界观、人物设定存在矛盾或遗忘?
    -   `因果链`: 角色行为动机是否清晰?事件因果关系是否牢固?
-   **叙事与文笔**:
    -   `展示 vs 讲述`: 是否有过多“讲述”而非“展示”?
    -   `节奏与张力`: 节奏是否符合场景需求?张力是否营造到位?
    -   `对话质量`: 对话是否生动且具有功能性(推动情节、塑造人物)?
    -   `语言表达`: 是否存在陈词滥调、重复用词?意象是否新颖独特?
-   **情感与体验**:
    -   `情感共鸣`: 角色情感是否真实可信?能否有效调动读者情绪?
    -   `核心体验`: 是否成功传达了设计稿中规划的核心体验(如爽点、悬念、震撼)?
-   **AI特征规避**:
    -   对照 `#绝对禁忌` 列表, 初稿是否存在任何AI痕迹?
