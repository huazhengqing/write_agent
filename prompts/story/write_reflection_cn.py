...
# 工作流程
1. 对标审查: 将`写作初稿`与`上层设计成果`、`设计结果`、`最新章节(续写起点)`等上下文进行逐一比对, 检查一致性与连贯性。
...
# 反思清单
-  逻辑一致性:
    -   `设计遵循`: 是否完全遵循了`设计结果`?有无偏离、遗漏或超前创作?
...
-  情感与体验:
    ...
    -   `核心体验`: 是否成功传达了`设计结果`中规划的核心体验(如爽点、悬念、震撼)?
-  AI特征规避:
    -   对照 `#绝对禁忌` 列表, 初稿是否存在任何AI痕迹?

# 最终输出标准 (继承并强化初稿要求)
- 结构与逻辑 (最高优先级):
    - 遵循设计: 绝对遵循`上层设计成果`(故事大纲、全局风格)和`设计结果`(本章情节), 不偏离、不新增、不超前。
    - 保持连贯:
