...
```markdown
# 摘要标题
- 格式: `# 摘要: [层级与位置]: [标题]` (例如: `# 摘要: [第一幕 | 第三章]: 风起于青萍之末`)
...
- 格式: `- [类型(悬念/伏笔)] [状态(新埋下/已回收)]: [具体描述] (关联: [角色/物品/事件])`
- 类型定义:
...
- ID规范: Mermaid图中的节点ID必须使用实体的英文/拼音缩-写或有意义的简称 (例如, '地点A' -> 'loc_a', '关键事件' -> 'key_event')。禁止使用中文、特殊字符或无意义的单字母ID。
- 在图下方, 可用列表补充文字说明, 格式为: `[地点名称]: - [事件1], - [事件2]`

## 世界观与设定
- 提炼文本中揭示或更新的、作为故事背景框架的世界规则、地理、历史、势力等宏观设定。
- 格式: `- [设定类别]: [具体描述]`

## 关键物品与概念
- 列出在本段文本中新出现或被重点使用的、直接推动情节或塑造角色的具体物品或专有概念。
- 物品: 指具体、有形的道具, 如“虚空之镜”。
- 概念: 指抽象的、被命名的设定, 如“血之契约”、“心剑合一境界”等。
- 格式: `- [物品/概念名称]: [功能、意义或在本次剧情中的作用]`
```
...
```mermaid
graph TD
    char_a[角色A] -- "彻底敌对" --> char_b[角色B];
    char_a -- "产生嫌隙" --> char_c[角色C];
    char_b -- "隶属于" --> secret_org[神秘组织];
```
...
```mermaid
graph TD
    subgraph loc_initial["[初始地点]"]
        event_outbreak["灾难事件爆发地"];
    end
    subgraph loc_key["[关键地点]"]
        event_confrontation["对峙与战斗"];
        event_discovery["发现关键证物"];
        event_escape["负伤逃脱"];
    end
    event_outbreak --> event_confrontation;
    event_confrontation --> event_discovery;
    event_discovery --> event_escape;
```
- [初始地点]: - [灾难事件]的爆发地, 是整个事件的起点。
- [关键地点]: - 对峙与战斗 - 发现关键证物 - 负伤逃脱
