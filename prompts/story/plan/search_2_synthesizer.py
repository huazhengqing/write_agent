...
# 任务
将`研究任务清单草案`(Markdown)精确转换为最终的JSON任务树。

# 原则
- 忠实转换: 严格遵循`研究任务清单草案`进行格式化, 不进行任何创造。
- 格式精确: 输出必须是符合`#输出格式`的纯JSON。

# 工作流程
1.  解析: 解析`研究任务清单草案`中的`### 任务清单草案`和`### 依赖关系`。
2.  构建: 为每个任务生成ID, 并根据`依赖关系`填充`dependency`字段。
3.  组合: 将所有任务组合成一个JSON对象, `reasoning`字段引用`研究任务清单草案`中的`### 研究计划分析`。
