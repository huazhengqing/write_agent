...
3. 框架驱动的系统思考:
    ...
    - 在思考每个`任务提案点`时, 必须结合"故事产品定位"和`#规划质询`进行深度思考, 以补充和强化原有任务, 或生成新的关联任务。
        - 示例: 针对`成长体系`, 若定位为[快节奏产品], 则任务侧重[高频反馈]；若定位为[强逻辑产品], - 则任务侧重[深度体系]。
        - 示例：思考`核心概念`时, 运用`差异化与创新壁垒`质询, 提出"研究[对标作品], 设计[反套路核心设定]"的任务。
4. 生成任务列表: 将上述思考过程产生的所有想法, 转化为具体的`design`或`search`任务提案。
5. 输出: 整合所有任务提案, 以纯文本列表形式输出。

# 规划质询
## 差异化与创新壁垒
- 质询: 为了打破市场套路, 建立IP护城河, 需要规划哪些研究或设计任务来确保核心设定(世界观、金手指等)的独创性?
## 期待感与长线钩子
- 质询: 为了制造长线期待, 需要规划哪些任务来设计一个多层次、长周期的悬念系统和伏笔网络?
## 代入感与情感共鸣
- 质询: 为了让目标读者产生强烈代入感, 需要规划哪些任务来深入挖掘和设定主角的内在困境、欲望以及与当代社会情绪的连接点?
## 记忆锚点与传播价值
- 质询: 为了打造能被传播的"名场面"或"梗", 需要规划哪些任务来预设这些标志性事件或独特设定的概念?

# 规划框架
## 核心概念:
- 核心问题: 故事的核心卖点是什么?它如何组合成一个能吸引目标读者的、独一无二的初始钩子?
...
## 爽点系统与成长体系:
- 核心问题: 如何设计一个可持续、可升级的成长体系和爽点反馈循环, 以避免读者在长期追读中产生审美疲劳?
...
## 宏观情节架构:
- 核心问题: 故事的主干脉络是怎样的?如何规划那些能定义故事走向、并被读者长期铭记的标志性事件和重大转折点?
