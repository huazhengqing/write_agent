...
# 目标优化原则 (重构任务时必须遵循)
- 上下文驱动: 必须利用所有可用上下文 (如 `上层设计成果`, `历史情节概要`, `设计方案` 等) 来补充和具体化每个子任务的 `goal`。
- 指令与内容分离: `goal` 必须是清晰的“做什么”的指令, 而非“是什么”的创作内容。
- 禁止无据创造: 严禁在 `goal` 中创造上下文中不存在的情节或设定。当上下文不足时, 使用更抽象的指令。
- 保持核心: 优化 `goal` 时不得修改任务的核心范畴 (如层级、核心目的)。
...
3. 制定优化策略: 针对每个不足之处, 构思具体的优化方案。
    - 示例1: “初步规划缺少对新概念‘[核心概念]’的背景资料搜集。新方案将增加一个`search`任务: '搜集关于‘[核心概念]’的[相关领域]设定', 并将其作为`design`任务的前置依赖。”
    - 示例2: “初步规划中的`design`任务'设计[某个实体]'过于模糊。新方案将把它细分为'design: [该实体]的[方面A]'、'design: [该实体]的[方面B]'和'design: [该实体]的[方面C]'三个子任务。”
...
- 依赖关系: `dependency` 是否准确反映了同层级任务间的逻辑先后顺序?
