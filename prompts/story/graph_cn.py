...
- 输出JSON 1:
    {{
        "nodes": [
            ...
            {{"id": "[角色名]", "label": "角色", "properties": {{"name": "[角色名]", "背景": "[背景设定]"}}}},
...
kg_extraction_prompt_search = """
# 任务
从“外部搜索结果”中提取实体、属性和关系, 输出为纯JSON。
...
- 格式: 纯JSON对象 `{{"nodes": [...], "edges": [...]}}`。
- 节点ID: 必须使用实体原始中文名。
- 属性提取: 从文本中提取关键属性 (如 `weight`, `material`, `era`) 并放入 `properties`。
- 量化数据: 尽可能提取数字和单位作为属性, 例如 `{"weight": "10kg", "length": "5m"}`。
- 来源节点化: 将信息来源 (URL, 文献名) 建立为独立的 `来源` 节点, 并使用 `来源于` 关系连接到实体。
- 别名处理: 别名或昵称需统一为标准名ID。
- 约束: 节点ID唯一, 关系必须连接已定义节点。

# 示例
- 输入事实:
    "根据资料[来源URL], [物品A]是[某个时期]的一种物品, 由[某种材料]制成, 重量为[10kg], 其特征是[特征1]和[特征2]。"
- 输出JSON:
    {{
        "nodes": [
            {{"id": "[物品A]", "label": "物品", "properties": {{"name": "[物品A]", "era": "[某个时期]", "material": "[某种材料]", "weight": "10kg"}}}},
            {{"id": "[特征1]", "label": "概念", "properties": {{"name": "[特征1]"}}}},
            {{"id": "[特征2]", "label": "概念", "properties": {{"name": "[特征2]"}}}},
            {{"id": "[来源URL]", "label": "来源", "properties": {{"name": "[来源URL]", "url": "[来源URL]"}}}}
        ],
        "edges": [
            {{"source": "[物品A]", "target": "[特征1]", "label": "特征是", "properties": {{}}}},
            {{"source": "[物品A]", "target": "[特征2]", "label": "特征是", "properties": {{}}}},
            {{"source": "[物品A]", "target": "[来源URL]", "label": "来源于", "properties": {{}}}}
        ]
    }}

---
现在, 为以下外部搜索结果提取图谱数据。禁止任何解释, 只输出纯JSON。
{context_str}
"""
