...
2.  分析现状: 回顾`已有知识与信息缺口`与`最近的思考历史`, 明确核心信息缺口。
...
- 示例:
{
  "thought": "[目标优化]: 原始焦点是'研究[某个角色]的背景', 根据`设计方案`中的'角色名为[角色名]', 优化为'研究[角色名]的背景故事'。\\n[现状分析]: 上一轮搜索已明确[角色名]的[职业/身份], 但其[核心动机]不明, 这是核心信息缺口。\\n[策略制定]: 本轮需深入挖掘其[核心动机]和[关键经历]。\\n[行动计划]: 我将从【因果关系】和【时序演变】两个维度进行探索。为验证'[角色名]是为[某个动机]而行动'的假设, 我会先搜索其[关键经历]以寻找[潜在冲突], 同时搜索[相关背景]作为参照, 以排除一般性动机。",
  "queries": [
    "[角色名] [关键经历] [关键词]",
    "[相关背景] [事件类型]",
    "[职业/身份] [心理分析]"
...
# 任务
以`核心研究摘要`为框架, `补充材料`为细节, 整合所有研究材料, 撰写一份结构化的 Markdown 报告。报告需服务于“原始任务”的创作目标。
...
### 1. 核心发现
- 根据`核心研究摘要`, 用1-3个要点概括最重要的结论。

### 2. 详细分析
- 以`核心研究摘要`为骨架展开论述。
- 引用`补充材料`中的内容作为证据, 并注明来源 (例如: `[来源: URL]`)。
