...
- 专门处理: `卷`到`幕`的层级划分。
- 当前任务是`写一卷`。
- 触发: 在`refiner`为`幕`生成最终的Markdown规划蓝图后被调用。
- 步骤: 这是`Proposer -> Critic -> Refiner -> Formatter`四步工作流的最后一步。
- 职责: 将一份`幕`的Markdown规划蓝图, 严格转换为纯JSON格式的子任务。
"""
...
# 任务
将`幕`的`最终规划蓝图`Markdown内容，严格转换为一个纯JSON对象。
...
- 动作:
    - 将`当前任务`(`写一卷`)作为根对象, 解析`最终规划蓝图`中的`幕`单元。
    - 为每个`幕`单元创建JSON对象, 并根据`#原则`和`#字段映射规则`填充信息。
    - 将所有子任务JSON对象组成的列表, 赋值给根对象的`sub_tasks`字段。
    - 在根对象的`reasoning`字段中总结本次转换生成的子任务数量。

# 字段映射规则
- `标题` 与 `核心使命与情节目标` -> `goal` (格式: "标题: 核心使命与情节目标")
...
- `task_type`: (字符串) 'write'。
- `hierarchical_position`: (字符串) 任务层级位置 (如: '第一卷 第一幕')。
