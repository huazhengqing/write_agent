...
# 任务
为`当前卷`的设计任务(情节架构、核心冲突、结构节奏等)构建一份专业的`情节架构指南`。这份指南的核心是方法论与规则, 用于指导后续的`设计草稿`工作, 而非产出具体的情节方案。
...
- 动作:
    - 基于本卷在全书中的使命, 提炼出本卷需要解决的核心矛盾、达成的核心目标, 并制造一个能让读者满足的阶段性高潮。
    - 将其定义为一个具体的、需要通过本卷的情节结构设计来解决的根本性问题 (例如: "如何设计一个'螳螂捕蝉黄雀在后'的连环反转结构, 来完成主角的复仇目标？")。

## 3. 情节创新引擎 (仅在“创造”型任务中完整执行)
...
- **原始任务**
- 任务本质分析
- **核心情节架构**: (此部分根据任务性质动态生成, 可能包含`情节创新引擎`、`整体结构模型`、`关键情节节点`等部分或全部内容, 或更侧重于具体事件链和节奏点)
- **结构设计原则**
- **风险**
- **规避项(负面清单)**
- **成功标准**
