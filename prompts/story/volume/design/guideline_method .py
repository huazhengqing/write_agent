...
# 工作流程
## 分析现状
- 目标: 全面理解所有上下文, 识别潜在冲突与机遇, 确保新设计与现有方案兼容。
- 动作:
    - 检查所有上下文信息, 确保无遗漏。
    - 识别信息间的关联, 建立对项目的整体认知。
    - 找出潜在的冲突、机遇或未明确的关键点。
...
- 动作:
    - 对`当前任务`的表面目标进行连续追问, 直至找到其根本目的, 即本卷在推动主线情节、完成角色弧光、提供阶段性满足感方面的核心使命。
    - 结合此使命、目标读者需求与市场竞争环境, 提炼出决定本卷成败的核心矛盾。
    - 将核心矛盾表述为一个需要解决的根本问题。 (例如: "如何原创一种方法论, 来设计一个既能让主角完成实力跃迁, 又能引入一个更强大敌人的'秘境探索'卷?")
