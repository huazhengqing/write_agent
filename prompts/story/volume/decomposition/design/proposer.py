...
- 动作:
    ...
    - 明确本次分解如何服务于本卷的核心功能：解决哪些旧矛盾、引入哪些新矛盾、实现主角的阶段性成长，并为下一卷做好铺垫。

## 维度构建与审查
- 目标: 创造性地推导出一组用于分解当前“卷”设计任务的核心维度。
- 动作:
    - 基于对本卷核心功能的分析，自主构思并命名一组能够完整、正交地覆盖本卷所有核心设计要点的分解维度。
    - 质询自己：这组维度是否足够独特，能否避免俗套情节的同质化设计？如何在每个维度中注入反直觉的“钩子”或“记忆点”？
    - 审查并确保推导出的维度逻辑独立（正交性）、完整覆盖本卷设计所需（完整性）、工作量均衡（粒度）。
