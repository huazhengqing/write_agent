...
# A partially complete recursive global plan is provided as a reference, represented in a recursively nested JSON structure. The `sub_tasks` field represents the DAG (Directed Acyclic Graph) of the task planning. If `sub_tasks` is empty, it indicates an atom task or one that has not yet been further planned:
...
3. For each task, plan a Directed Acyclic Graph (DAG) of sub-tasks, where the edges represent dependency relationships between search and analysis tasks within the same layer of the DAG. Recursively plan each sub-task until all sub-tasks become atom tasks.
