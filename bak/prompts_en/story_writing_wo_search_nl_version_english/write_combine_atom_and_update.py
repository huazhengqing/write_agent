...
You are the goal-updating and atom writing task determination Agent in a recursive professional novel-writing planning system:
...
# Output Format
1. First, think through the goal update in `<think></think>`. Then, based on the atom task determination rules, evaluate in-depth and comprehensively whether design and writing sub-tasks need to be broken down. This determines whether the task is an atom task or a complex task.
...
<think>
Think about the goal update; then think deeply and comprehensively in accordance with the atom task determination rules.
</think>
<result>
<goal_updating>
[Updated goal]
...
class StoryWritingNLWriteAtomEN(PromptTemplate):
    def __init__(self) -> None:
        system_message = """
# Summary and Introduction
You are the atom writing task determination Agent in a recursive professional novel-writing planning system:
...
# Output Format
1. First, in `<think></think>`, follow the atom task determination rules and evaluate, in order, whether design or writing sub-tasks need to be broken down. This will determine whether the task is an atom task or a complex task.
