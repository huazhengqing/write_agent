...
class MemoryGraph:
    ...
    def _retrieve_nodes_from_data(self, data, filters):
        ...
        entity_type_map = {
            self._clean_cypher_identifier(k.lower()): self._clean_cypher_identifier(v.lower())
    ...
    def _search_graph_db(self, node_list, filters, limit=100):
        ...
        for node in node_list:
            ...
            if filters.get("agent_id"):
                ...
                WITH COLLECT(n) AS nodes, n.embedding AS n_embedding
                CALL node_similarity.cosine_pairwise("embedding", nodes, nodes)
                YIELD node1, node2, similarity
                WITH node1 AS n, similarity
    ...
    def _clean_cypher_identifier(self, name: str) -> str:
        """
        ...
        return cleaned_name if cleaned_name else "unnamed_entity"
    
    def _remove_spaces_from_entities(self, entity_list):
        for item in entity_list:
            item["source"] = self._clean_cypher_identifier(item["source"].lower())
            # item["source"] = item["source"].lower().replace(" ", "_")
            # Use the sanitization function for relationships to handle special characters
            item["relationship"] = self._clean_cypher_identifier(sanitize_relationship_for_cypher(item["relationship"].lower().replace(" ", "_")))
            # item["relationship"] = sanitize_relationship_for_cypher(item["relationship"].lower().replace(" ", "_"))
            item["destination"] = self._clean_cypher_identifier(item["destination"].lower())
