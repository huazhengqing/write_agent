...
# 原则: 
1.  实体: 主语、宾语需明确。
    - 类型: 优先提取命名实体 (人名、地名、组织名)、专业术语、具体概念。避免提取过于宽泛或无意义的实体 (如 "文本", "信息")。
    - 规范: 同一实体指称需统一。
    - 完整: 实体名称保持完整。
2.  关系: 精确描述实体间联系。
    - 具体: 避免模糊关系, 优先使用具体动词。
...
4.  事实: 忠于原文, 避免推断和常识。
5.  信息量: 提取的三元组应包含有价值的信息。
    - **不提取**: 对于无信息量、常识性或非事实性的陈述 (例如 "这是一个测试文本", "你好"), 不应提取三元组。
    - **不提取**: 对于代码块、Mermaid图表、纯数据(如JSON、表格)的结构本身, 不提取三元组, 除非文本对这些结构进行了描述。
