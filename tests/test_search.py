...
async def test_search_with_searxng_success(monkeypatch):
    """【单元测试】测试 _search_with_searxng 成功时能正确格式化结果。"""
    logger.info("--- 测试 _search_with_searxng (成功) ---")
    ...
    from utils.search import _search_with_searxng
    result = await _search_with_searxng("test query", max_results=1)
    ...
    logger.success("--- _search_with_searxng (成功) 测试通过 ---")


def test_search_with_ddg_success(monkeypatch):
    """【单元测试】测试 _search_with_ddg 成功时能正确格式化结果。"""
    logger.info("--- 测试 _search_with_ddg (成功) ---")
    ...
    from utils.search import _search_with_ddg
    result = _search_with_ddg("test query", max_results=1)

    assert "DDG Test Title" in result
    assert "http://ddg.com" in result
    assert "DDG summary." in result
    assert "搜索结果 1" in result
    logger.success("--- _search_with_ddg (成功) 测试通过 ---")


async def test_scrape_static_success(monkeypatch):
    """【单元测试】测试 _scrape_static 成功抓取和提取。"""
    logger.info("--- 测试 _scrape_static (成功) ---")
    mock_response = MagicMock()
    ...
    from utils.search import _scrape_static
    result = await _scrape_static("http://static.com")

    assert result == long_static_content
    mock_trafilatura.assert_called_once()
    logger.success("--- _scrape_static (成功) 测试通过 ---")


async def test_scrape_dynamic_success(monkeypatch):
    """【单元测试】测试 _scrape_dynamic 成功抓取和提取。"""
    logger.info("--- 测试 _scrape_dynamic (成功) ---")
    ...
    from utils.search import _scrape_dynamic
    result = await _scrape_dynamic("http://dynamic.com")

    assert result == long_dynamic_content
    mock_trafilatura.assert_called_once()
    mock_page.goto.assert_called_once()
    mock_browser.close.assert_called_once()
    logger.success("--- _scrape_dynamic (成功) 测试通过 ---")
...
async def test_scrape_content_truncation(monkeypatch):
    ...
    # 模拟抓取策略以返回我们的长文本
    mock_scrape_static = AsyncMock(return_value=long_text)
    monkeypatch.setattr("utils.search._scrape_static", mock_scrape_static)
    # 阻止动态抓取器运行
    monkeypatch.setattr("utils.search._scrape_dynamic", AsyncMock(return_value=None))
...
async def test_web_search_failure_raises_error(monkeypatch):
    ...
    monkeypatch.setattr("utils.search._search_with_searxng", mock_searxng)

    mock_ddg = MagicMock(side_effect=Exception("DDG failed"))
    monkeypatch.setattr("utils.search._search_with_ddg", mock_ddg)
...
async def test_scrape_failure_returns_empty_string(monkeypatch, caplog):
    ...
    mock_scrape_static = AsyncMock(side_effect=Exception("Static scrape failed"))
    monkeypatch.setattr("utils.search._scrape_static", mock_scrape_static)

    mock_scrape_dynamic = AsyncMock(side_effect=Exception("Dynamic scrape failed"))
    monkeypatch.setattr("utils.search._scrape_dynamic", mock_scrape_dynamic)
    ...
    mock_scrape_static.assert_called_once_with(url)
    mock_scrape_dynamic.assert_called_once_with(url)
...
async def test_scrape_fallback_strategy(monkeypatch, caplog):
    ...
    mock_scrape_static = AsyncMock(return_value="")
    monkeypatch.setattr("utils.search._scrape_static", mock_scrape_static)

    # 模拟动态抓取成功
    mock_scrape_dynamic = AsyncMock(return_value=expected_content)
    monkeypatch.setattr("utils.search._scrape_dynamic", mock_scrape_dynamic)
