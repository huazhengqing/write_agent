...
class IdeaOutput(BaseModel):
    ...
    input_brief: str = Field(description="关于故事背景、主角、世界观等关键设定的输入指引。")
    constraints: str = Field(description="创作时需要避免的内容，例如特定的情节、元素或主题。")
    acceptance_criteria: str = Field(description="衡量项目初步成功的标准，例如需要完成哪些章节或内容。")
...
4.  `input_brief`: **(对应思考流程2.2, 2.4)** 详细阐述“主角设定”（包含背景、动机、性格、金手指、以及Lie/Want/Need构成的成长弧光）和“世界观核心设定”（包含核心'What if'问题、独特法则、背景谜团）。
5.  `constraints`: **(对应思考流程2.1, 2.5)** 包含“竞品分析与差异化策略”和“市场风险与规避策略”（明确要避免的常见套路和读者"毒点"）。
6.  `acceptance_criteria`: **(对应思考流程2.4)** 设定项目启动的验收标准，即“黄金三章（开篇三章）的核心情节构思”，需明确每一章的钩子、冲突和爽点。
