...
1.  **定位下一个子任务**：通过对比`父任务的规划草案`和`整体规划(任务树)`，找出下一个待执行的子任务。
...
## 1. 定向 (Direction)
- **定位目标任务**:
    - **动态分析**：将`父任务的规划草案`视为一个确保项目收敛的**最小目标清单**，而不是严格的执行序列。你的首要任务是综合分析`整体规划(任务树)`中已完成和正在进行的子任务、所有`设计方案`以及`全局状态摘要`。
    ...
    - **完成状态判断**：只有当你判断`父任务`的所有核心目标（包括`规划草案`中蕴含的以及动态演化出的）均已通过`整体规划(任务树)`中的子任务充分实现时，才认为该父任务已完成。在这种情况下，你的任务是报告完成状态（输出`null`），而不是创建新任务。
