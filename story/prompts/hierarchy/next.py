...
## 1. 定向 (Direction)
- **定位目标单元**: 分析`整体规划(任务树)`和`子单元地图`，定位到下一个待规划的子单元。
    - 确定`父任务`下最新完成的子单元: 
...
- **完成状态判断**: 如果`子单元地图`中的所有单元都已在`整体规划(任务树)`中创建或完成，则意味着该父任务下的所有子单元都已规划完毕。在这种情况下，你的任务是报告完成状态，而不是创建新任务。
