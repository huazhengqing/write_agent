...
async def hierarchy(task: Task) -> Task:
    system_prompt, user_prompt = load_prompts(task.category, "hierarchy", "system_prompt", "user_prompt")
...
async def hierarchy_reflection(task: Task) -> Task:
    ...
    if os.getenv("deployment_environment") == "test":
        ...
        system_prompt, user_prompt = load_prompts(task.category, "hierarchy_reflection", "system_prompt", "user_prompt")
        context = await get_story_rag().get_context(task)
