...
async def review_design(task: Task) -> Task:
    system_prompt, user_prompt = load_prompts(task.category, "review_design", "system_prompt", "user_prompt")
...
async def review_write(task: Task) -> Task:
    system_prompt, user_prompt = load_prompts(task.category, "review_write", "system_prompt", "user_prompt")
    context = await get_story_rag().get_context(task)
    context.update({
